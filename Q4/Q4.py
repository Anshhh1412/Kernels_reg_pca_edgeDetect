        ...
        for f in range(F):
            ...
            for j in range((H-HH+1)//stride):
                for k in range((W-WW+1)//stride):
                    # print(out[i,f,j*stride:j*stride+HH,k*stride:k*stride+WW].shape)
                    # print(x[i,j*stride:j*stride+HH,k*stride:k*stride+WW].shape)
                   
                    out[i,f,(j*stride)+HH//2,(k*stride)+ WW//2] = np.ceil(np.sum(np.multiply(x[i,j*stride:j*stride+HH,k*stride:k*stride+WW],w[f])))

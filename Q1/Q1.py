...
def gauss_kernel(a,b):
    ...
    ######START: TO CODE########(1/(2*(sigma**2)))
    out = np.ndarray(0)
    x1 = np.sum(a**2, axis=-1)
    y1 = np.sum(b**2, axis=-1)
    
    out =np.exp(-(1/(2*(sigma**2)))*(x1[:,None] + y1[None,:] - 2*np.dot(a,b.T)))
...
def rbf_kernel(a,b):
    ...
    out =np.exp(-gamma*(x1[:,None] + y1[None,:] - 2*np.dot(a,b.T)))
...
def laplace_kernel(a,b):
    ...
    n = a.shape[0]
    out = np.zeros((n,n))
    out =np.exp(-(1/sigma)*np.sum(np.abs(a[:,None]-b[None,:]),axis=2))
